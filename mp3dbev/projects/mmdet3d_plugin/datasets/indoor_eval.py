    ...
    for img_id in range(len(dt_annos)):
        ...
        for i in range(len(det_anno['labels_3d'])):
            ...
            bbox =  LiDARInstance3DBoxes(
                det_anno['boxes_3d'][i].tensor[:, :7],
                box_dim=7,
                origin=(0.5, 0.5, 0)).convert_to(box_mode_3d)
        ...
        if gt_anno['gt_num'] != 0:
            gt_boxes = box_type_3d(
                ...
                origin=(0.5, 0.5, 0)).convert_to(box_mode_3d)
            # newgt_anno = torch.from_numpy(gt_anno['gt_boxes_upright_depth'])
            # gt_boxes = LiDARInstance3DBoxes(
            #     torch.cat((newgt_anno, torch.zeros(newgt_anno.shape[0], 2)),1),
            #     box_dim=9,
            #     origin=(0.5, 0.5, 0)).convert_to(box_mode_3d)
            # gt_boxes = box_type_3d(
            #     gt_anno['gt_boxes_upright_depth'],
