...
from models.model_bev import VLNBert, Critic
from models.ops import pad_tensors_wgrad

from utils.data import CamFeatures
...
class GMapNavAgent(Seq2SeqAgent):
    ...
    def _cam_feature_variable(self, obs):
        '''
        output -> camera_image_feature; candi_loc
        '''
        batch_cam_fts = []
        batch_scan = []
        batch_vp = []
        ...
        batch_argscamera = []
        for i, ob in enumerate(obs):
            if ob['cam_feature'] is None:
                batch_cam_fts = None
            else:
                batch_cam_fts.append(torch.from_numpy(ob['cam_feature']))
            batch_scan.append(ob['scan'])
            batch_argscamera.append(ob['argscamera'])
            batch_vp.append(ob['viewpoint'])
            batch_candis.append(ob['candidate'])
        if ob['cam_feature'] is None:
            batch_cam_fts = None
        else:    
            batch_cam_fts = torch.stack(batch_cam_fts, 1).squeeze(2).permute(0, 2, 1, 3).cuda() # (num_cam, 196, bs, 768)
        return {
            'cam_fts': batch_cam_fts, 'candis': batch_candis,
            'scan': batch_scan,
            'argscamera': batch_argscamera,
            'vp': batch_vp,
        }
    ...
    def _nav_bevmap_variable(self, obs, bevmaps):
        if bevmaps is not None:
            # [stop] + gmap_vpids
            batch_size = len(obs)
            ...
            batch_gmap_pair_dists, batch_gmap_visited_masks = [], []
            batch_no_vp_left = []
            for i, gmap in enumerate(bevmaps):
                visited_vpids, unvisited_vpids = [], []                
                for k in gmap.node_positions.keys():
                    if self.args.act_visited_nodes:
                        if k == obs[i]['viewpoint']:
            ...
            return {
                'bevmap_vpids': batch_gmap_vpids, 'bevmap_img_embeds': batch_gmap_img_embeds, 
                'bevmap_step_ids': batch_gmap_step_ids, 'bevmap_pos_fts': batch_gmap_pos_fts,
                'bevmap_visited_masks': batch_gmap_visited_masks, 
                'bevmap_pair_dists': gmap_pair_dists, 'bevmap_masks': batch_gmap_masks,
                'bev_no_vp_left': batch_no_vp_left,
            }
        else:
            return {
                'bevmap_vpids': None, 'bevmap_img_embeds': None, 
                'bevmap_step_ids': None, 'bevmap_pos_fts': None,
                'bevmap_visited_masks': None, 
                'bevmap_pair_dists': None, 'bevmap_masks': None,
                'bev_no_vp_left': None,
            }

    def _nav_bevvp_variable(self, obs, bevvp_embeds, cand_vpids, batch_candi_bev_len, nav_types):
        batch_size = len(obs)
        # add [stop] token
        bevvp_embeds = torch.cat(
            [torch.zeros_like(bevvp_embeds[:, :1]), bevvp_embeds], 1
        )

        bevvp_nav_masks = torch.cat([torch.ones(batch_size, 1).bool().cuda(), nav_types == 1], 1)
        return {
            'bevvp_embeds': bevvp_embeds, # [8, 50, 768]
            'bevvp_masks': gen_seq_masks(batch_candi_bev_len+1), 
            'bevvp_nav_masks': bevvp_nav_masks, # [8, 50]
            'bevvp_cand_vpids': [[None]+x for x in cand_vpids],
        }
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        if self.args.bevglobal:
            bevmaps = [GraphMap(ob['viewpoint']) for ob in obs]
        else:
            bevmaps = None
        for i, ob in enumerate(obs):
            gmaps[i].update_graph(ob)
            if self.args.bevglobal:
                bevmaps[i].update_graph(ob)
        ...
        for t in range(self.args.max_action_len):
            ...
            if self.args.bevglobal:
                for i, bevmap in enumerate(bevmaps):
                    if not ended[i]:
                        bevmap.node_step_ids[obs[i]['viewpoint']] = t + 1
            ...
            # bev representation
            cam_inputs = self._cam_feature_variable(obs)
            bev_embeds, batch_candi_bev, candi_id, finebevembs, bevpoints, candi2bev_tmp = self.vln_bert('cam_fts', cam_inputs)
            
            # for bev embeds + object
            batch_candi_bev_len = []
            batch_bevvp_navtype = []
            for i, ob in enumerate(obs):
                bevvp_navtype = []
                batch_candi_bev_len.append(len(batch_candi_bev[i]))
                bevvp_navtype = torch.LongTensor([1 for k in  range(len(batch_candi_bev[i]))])
                batch_bevvp_navtype.append(bevvp_navtype)
            batch_bevvp_navtype = pad_sequence(batch_bevvp_navtype, batch_first=True, padding_value=0).cuda()

            batch_candi_bev_len = torch.LongTensor(batch_candi_bev_len).cuda()
            batch_candi_bev = pad_tensors(batch_candi_bev).cuda()
            bev_inputs = {
                'batch_candi_bev': batch_candi_bev,
                'batch_candi_bev_len': batch_candi_bev_len,
                'pano_inputs': pano_inputs,
            }
            bevvp_embeds, bevvp_masks = self.vln_bert('bev_fts', bev_inputs)
            avg_bevvp_embeds = torch.sum(bevvp_embeds * bevvp_masks.unsqueeze(2), 1) / \
                              torch.sum(bevvp_masks, 1, keepdim=True)
            ...
            if self.args.bevglobal:
                for i, bevmap in enumerate(bevmaps):
                    if not ended[i]:
                        # update visited node
                        i_vp = obs[i]['viewpoint']
                        bevmap.update_node_embed(i_vp, avg_bevvp_embeds[i], rewrite=True)
                        # update unvisited nodes
                        for j, candis in enumerate(cam_inputs['candis'][i]):
                            if not bevmap.graph.visited(candis['viewpointId']):
                                bevmap.update_node_embed(candis['viewpointId'], bevvp_embeds[i,j])
            
            # navigation policy
            # gmaps = bevmaps
            nav_inputs = self._nav_gmap_variable(obs, gmaps) # global
            nav_inputs.update(
                self._nav_bevmap_variable(obs, bevmaps)
            )
            nav_inputs.update(
                self._nav_bevvp_variable(
                    obs, bevvp_embeds, pano_inputs['cand_vpids'], 
                    batch_candi_bev_len, batch_bevvp_navtype,
                    )
            )
            nav_inputs.update(
            ...
            nav_inputs.update({
                'finebevembs': finebevembs,
                'bevpoints': bevpoints,
                'candi2bev_tmp': candi2bev_tmp,
                'candis': cam_inputs['candis'],
            })
            nav_outs = self.vln_bert('navigation', nav_inputs)

            global_targets = self._teacher_action_r4r(
                    obs, nav_inputs['gmap_vpids'], ended, 
                    visited_masks=nav_inputs['gmap_visited_masks']
                )
            local_targets = self._teacher_action_r4r(
                    obs, nav_inputs['vp_cand_vpids'], ended, 
                    visited_masks=None
                )
            # for global
            if self.args.onlybev:
                self.logs['global_d'].append( 0.0 )
                self.logs['local_d'].append( 0.0 )
                self.logs['ofuse_d'].append( 0.0 )
            else:
                global_d = (global_targets == torch.max(nav_outs['global_logits'], -1)[1])
                local_d = (local_targets == torch.max(nav_outs['local_logits'], -1)[1])
                ofuse_d = (global_targets == torch.max(nav_outs['ofused_logits'], -1)[1])
                self.logs['global_d'].append( global_d.sum()/len(global_d) )
                self.logs['local_d'].append( local_d.sum()/len(local_d) )
                self.logs['ofuse_d'].append( ofuse_d.sum()/len(ofuse_d) )
            if self.args.bevglobal:
                bev_global_d = (global_targets == torch.max(nav_outs['bev_logits'], -1)[1]) 
                self.logs['bev_global_d'].append( bev_global_d.sum()/len(bev_global_d) )
            else:
                self.logs['bev_global_d'].append( 0.0 )
            fuse_d = (global_targets == torch.max(nav_outs['fused_logits'], -1)[1])
            self.logs['fuse_d'].append( fuse_d.sum()/len(fuse_d) )
            # for local
            bev_local_d = (local_targets == torch.max(nav_outs['bevlocal_logits'], -1)[1])
            self.logs['bev_local_d'].append( bev_local_d.sum()/len(bev_local_d) )
            ...
            elif self.args.fusion == 'bev':
                nav_logits = nav_outs['bev_logits']
                nav_vpids = nav_inputs['bev_vpids']
            else:
