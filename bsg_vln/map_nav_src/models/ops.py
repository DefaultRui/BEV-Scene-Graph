...
def findindex(bev_grid, localgrid):
    x_grid, y_grid = torch.meshgrid(
        torch.arange(bev_grid).cuda(),
        torch.arange(bev_grid).cuda(),
        indexing='ij'
        )
    xy_grid = torch.stack((x_grid, y_grid), -1).view(-1, 2).cuda()
    center_grid = torch.tensor([bev_grid //2, bev_grid //2]).cuda()
    center_grid = center_grid[None].repeat(bev_grid ** 2, 1)
    index = torch.pow(xy_grid - center_grid, 2).sum(-1).sort(-1)[1][:localgrid**2].cuda()
    #!!! index.sort()[0] instead of index
    return index.sort()[0]