...
class GraphMap(object):
    ...
    def get_gmap_point_pos(self, cur_vp, gmap_vpids):
        gmap_point_pos = []
        for vp in gmap_vpids:
            if vp is None:
                gmap_point_pos.append(self.node_positions[cur_vp][:2])
            else:
                gmap_point_pos.append(self.node_positions[vp][:2])
        return np.array(gmap_point_pos)
