...
class CamFeatures(object):
    def __init__(self, img_ft_file, image_feat_size=768):
        self.image_feat_size = image_feat_size
        self.img_ft_file = img_ft_file
        self._feature_store = {}

    def get_image_feature(self, scan, viewpoint, cam_id, deg):
        key = '%s_%s_i%s_%s'%(scan, viewpoint, cam_id, deg)
        if key in self._feature_store:
            ft = self._feature_store[key]
        else:
            with h5py.File(self.img_ft_file, 'r') as f:
                ft = f[key][:, 1:, :self.image_feat_size].astype(np.float32)
                self._feature_store[key] = ft
        return ft
    
    def get_multi_images_feature(self, scan, viewpoint, cams=False):
        camfeats = []
        if cams:
            for cam_id in range(3):
                for deg in range(6):
                    camfeats.append(self.get_image_feature(scan, viewpoint, cam_id, deg))
        else:
            for deg in range(6):
                camfeats.append(self.get_image_feature(scan, viewpoint, '1', deg))
        return np.array(camfeats)
